...
// FUNÇÃO PARA EXPORTAR DADOS EM XLS
function exportarDadosXLS() {
  // MAPEAR EMPRESAS PELO ID
  ...
  // FORMATAR OS CONTATOS, ADICIONANDO O NOME DA EMPRESA RELACIONADA
  ...
  // CONVERTER OS ARRAYS EM ABAS DO XLS
  ...
  // SALVAR O ARQUIVO XLS COM NOMA BASEADO NA DATA ATUAL
...
// FUNÇÃO PARA NAVEGAÇÃO ENTRE AS TABS
...
// FUNÇÃO PARA ATUALIZAR ESTATÍSTICAS DOS CARDS NA ABA DE DASHBOARD
...
// FUNÇÃO PARA CARREGAR INSIGHTS NA ABA DE DASHBOARD
async function carregarInsights() {
  try {
    // TOP EMPRESAS POR ENGAJAMENTO
    const topEmpresasResponse = await authenticatedFetch('/api/dashboard/top-empresas');
    ...
    const evolucaoResponse = await authenticatedFetch('/api/dashboard/evolucao-mensal');
    ...
    // EFICIÊNCIA DO SISTEMAS
    const statsResponse = await authenticatedFetch('/api/dashboard/stats');
...
// FUNÇÃO PARA MAPEAR CONTATOS COM TREINAMENTO
async function mapContatosComTreinamento() {
  try {
    const response = await fetch('/api/dashboard/contatos-em-treinamento');
...
// FUNÇÃO PARA ATUALIZAR ESTATÍSTICAS DOS CARDS NA ABA DE EMPRESAS
...
// FUNÇÃO PARA RENDERIZAR OS CARDS DAS EMPRESAS DENTRO DA ABA DE EMPRESAS
      ...
      <div class="company-card">
        <div class="company-header">
          <div class="company-title">
            <h3>${empresa.razaoSocial || 'Empresa'}</h3>
            <!--<span class="company-type">${empresa.porte_empresa || 'Empresa'}</span>-->
          </div>
          <div class="company-btn-edit">
            <button onclick="abrirDetalhesEmpresa(${empresa.id})">
        ...
        <div class="company-info">
          <p>
            ...
            </strong> ${formatarCNPJ(empresa.cnpj)}
          </p>
          <p>
            <strong>
              <i class="fa-solid fa-envelope" ></i>
            </strong> ${empresa.email || 'N/A'}
          </p>
          <p>
            <strong>
              <i class="fa-solid fa-phone"></i>
            </strong> ${formatarTelefone(empresa.contato)}
          </p>
        </div>

        <div class="company-actions">
          <button onclick="visualizarContatosEmpresa(${empresa.id})">
            <i class="fa-solid fa-user"></i>  
            Contatos
          </button>
          <button onclick="abrirModalTreinamentosEmpresa(${empresa.id})">
...
// FUNÇÃO PARA VISUALIZAR OS CONTATOS DE CADA EMPRESA (BOTÃO CONTATOS)
function visualizarContatosEmpresa(empresaId) {
  const empresa = empresas.find(e => e.id === empresaId);
  if (!empresa) return;

  document.getElementById('modalTituloEmpresa').textContent = `Contatos - ${empresa.razaoSocial || 'Empresa'}`;
  ...
  document.getElementById('modalContatosEmpresa').style.display = 'flex';
...
// FUNÇÃO PARA RENDERIZAR CONTATOS DA EMPRESA DENTRO DA ABA DE EMPRESAS (MODAL BOTÃO CONTATOS)
    ...
    return `
          <div class="contact-item">
            ...
            <div class="contact-actions">
              <button onclick="abrirEditarContato(${contato.id})">
                <i class="fa-solid fa-pen-to-square"></i>  
                Editar
              </button>
              <button onclick="abrirModalTransferirFuncionario(${contato.id})">
                <i class="fa-solid fa-arrow-right-arrow-left"></i>
                Transferir
              </button>
              <button onclick="toggleAtivoContato(${contato.id})">
                ${contato.ativo === 0 ? '<i class="fa-solid fa-circle-check"></i> Ativar' : '<i class="fa-solid fa-ban"></i> Desativar'}
              </button>
              <button onclick="restartTreinamentoContato(${contato.id})" title="Reiniciar treinamento do contato">
                <i class="fa-solid fa-arrows-rotate"></i>
                Restart
              </button>
              <button onclick="exportarConversa(${contato.id})" title="Exportar conversa do WhatsApp">
...
// FUNÇÃO PARA ABRIR O MODAL DE EDITAR CONTATO
...
// FUNÇÃO PARA ATUALIZAR O SELECT DE TREINAMENTO
...
// FUNÇÃO PARA ABRIR O MODAL DE TRANSFERÊNCIA DE FUNCIONÁRIO ENTRE EMPRESAS (BOTÃO TRANFERIR)
function abrirModalTransferirFuncionario(contatoId) {
  const contato = contatos.find(c => c.id === contatoId);
  if (!contato) return;

  const empresaAtual = empresas.find(e => e.id === contato.empresaId);

  document.getElementById('transferirContatoId').value = contato.id;
  document.getElementById('transferirNomeFuncionario').textContent = contato.nome;
  document.getElementById('transferirEmpresaAtual').textContent = empresaAtual ? empresaAtual.razaoSocial : 'Empresa não encontrada';

  // Carregar empresas no select
  const selectNovaEmpresa = document.getElementById('transferirNovaEmpresa');
  selectNovaEmpresa.innerHTML = '<option value="">Selecione a nova empresa</option>';

  empresas.forEach(empresa => {
    if (empresa.id !== contato.empresaId) {
      const option = document.createElement('option');
      option.value = empresa.id;
      option.textContent = empresa.razaoSocial || empresa.razao_social;
      selectNovaEmpresa.appendChild(option);
    }
  });

  document.getElementById('modalTransferirFuncionario').style.display = 'flex';
}

// FUNÇÃO PARA FORMULÁRIO DE TRANSFERÊNCIA
document.getElementById('formTransferirFuncionario')?.addEventListener('submit', function (e) {
  e.preventDefault();

  const contatoId = document.getElementById('transferirContatoId').value;
  const novaEmpresaId = document.getElementById('transferirNovaEmpresa').value;

  if (!novaEmpresaId) {
    mostrarAlerta('Selecione a nova empresa.', 'error');
    return;
  }
  ...
  if (!confirm(`Confirma a transferência de ${contato.nome} para ${novaEmpresa.razaoSocial || novaEmpresa.razao_social}?`)) {
    return;
  }

  authenticatedFetch(`/api/contatos/${contatoId}/transferir-empresa`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    ...
    .then(() => {
      mostrarAlerta(`✅ ${contato.nome} foi transferido com sucesso!`);
    ...
    .catch(() => mostrarAlerta('Erro ao transferir funcionário.', 'error'));
});

// FUNÇÃO PARA FECHAR O MODAL DE TRANSFERÊNCIA
function fecharModalTransferir() {
  document.getElementById('modalTransferirFuncionario').style.display = 'none';
}

// FUNÇÃO PARA ATIVAR/DESATIVAR CONTATO
function toggleAtivoContato(contatoId) {
  const contato = contatos.find(c => c.id === contatoId);
  if (!contato) {
  ...
  authenticatedFetch(`/api/contatos/${contatoId}/toggle-ativo`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    ...
    .then(() => {
      mostrarAlerta(`Contato ${acao === 'ativar' ? 'ativado' : 'desativado'} com sucesso!`);
      carregarContatos().then(() => {
        atualizarEstatisticasMapeamento();
        if (empresaSelecionada) {
          contatosEmpresaSelecionada = contatos.filter(c => c.empresaId === empresaSelecionada.id);
    ...
    .catch(() => mostrarAlerta('Erro ao alterar status do contato.', 'error'));
}

// FUNÇÃO PARA REINICIAR TREINAMENTO DO CONTATO
function restartTreinamentoContato(contatoId) {
  ...
  authenticatedFetch(`/api/contatos/${contatoId}/restart-treinamento`, {
    method: 'POST'
  })
    .then(res => {
      if (!res.ok) throw new Error();
...
// FUNÇÃO PARA FECHAR MODAL DE CONTATOS DA EMPRESA
...
// FUNÇÃO PARA ABRIR O MODAL DE TREINAMENTO DA EMPRESA (BOTÃO TREINAMENTOS)
function abrirModalTreinamentosEmpresa(empresaId) {
  ...
  document.getElementById('modalTreinamentosEmpresa').style.display = 'flex';

  // Carregar treinamentos disponíveis e da empresa
  Promise.all([
    authenticatedFetch(`/api/empresas/${empresaId}/treinamentos/disponiveis`).then(r => r.json()),
    authenticatedFetch(`/api/empresas/${empresaId}/treinamentos/atribuidos`).then(r => r.json())
  ])
    .then(([treinamentosDisponiveis, treinamentosEmpresa]) => {
      ...
      <div class="treinamentos-empresa-container">
        <div class="treinamentos-disponiveis">
          <h4>
            <i class="fa-solid fa-book-open" style="color: #F1AE33"></i>
            ...
            ${treinamentosDisponiveis.length > 0 ? treinamentosDisponiveis.map((t, index) => `
              <div class="treinamento-item" style="animation-delay: ${index * 0.1}s">
                <div class="treinamento-info">
                  <div class="treinamento-title">
                    <strong>${t.nome}</strong>
                  </div>
                  <p>${t.modalidade} - ${t.carga_horaria}h</p>
                  <p>${t.instrutor_principal ? `${t.instrutor_principal}` : ''}</p>
                  <button onclick="atribuirTreinamento(${empresaId}, ${t.id})">
            ...
            ${treinamentosEmpresa.length > 0 ? treinamentosEmpresa.map((t, index) => `
              <div class="treinamento-item">
                <div class="treinamento-info">
                  ...
                  <p>${t.modalidade} - ${t.carga_horaria}h</p>
                  <p>${t.instrutor_principal ? `${t.instrutor_principal}` : ''}</p>
                  <button onclick="removerTreinamentoEmpresa(${empresaId}, ${t.id})">
...
// FUNÇÃO PARA FECHAR MODAL DE TREINAMENTOS
            ...
            <div class="form-section">
              ...
              <div class="form-group">
                ...
                <div class="input-wrapper">
                  ...
                  <input type="text" id="editRazaoSocial" value="${empresa.razaoSocial || ''}" required />
                </div>
              </div>
              <div class="form-group">
                <label>CNPJ</label>
                <div class="input-wrapper">
                  ...
                  <input type="text" id="editCnpj" placeholder="00.000.000/0000-00" value="${empresa.cnpj || ''}" />
                </div>
              </div>
              <div class="form-group">
                <label>Email</label>
                <div class="input-wrapper">
                  ...
                  <input type="email" id="editEmail" placeholder="empresa@exemplo.com" value="${empresa.email || ''}" />
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="input-wrapper">
                  ...
                  <input type="text" id="editTelefone" placeholder="(00) 00000-0000" value="${empresa.contato || ''}" />
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="input-wrapper">
                  ...
                  <input type="text" id="editEndereco" placeholder="Rua, número, bairro" value="${empresa.endereco || ''}" />
                </div>
              </div>
              <div class="form-group">
                <label >CEP</label>
                <div class="input-wrapper">
                  ...
                  <input type="text" id="editCep" placeholder="00000-000" value="${empresa.cep || ''}" />
                </div>
              </div>
              <div class="form-row">
                <div class="form-group">
                  ...
                  <div class="horario-funcionamento">
                    <div class="form-group">
                      <div class="input-wrapper">
                        ...
                        <input type="time" id="editHoraInicio" value="${empresa.horarioFuncionamento ? empresa.horarioFuncionamento.split('-')[0] : ''}" />
                      </div>
                    </div>
                    <span>
                    ...
                    <div class="form-group">
                      <div class="input-wrapper">
                        ...
                        <input type="time" id="editHoraFim" value="${empresa.horarioFuncionamento ? empresa.horarioFuncionamento.split('-')[1] : ''}" />
                      </div>
                    </div>
                  </div>
                  <small style="color: #9ca3af; font-size: 12px; display: block;">Deixe vazio para atendimento 24 horas</small>
                </div>
              </div>
...
// FUNÇÃO PARA FECHAR MODAL DE DETALHES DA EMPRESA
function fecharModalDetalhesEmpresa() {
  document.getElementById('modalDetalhesEmpresa').style.display = 'none';
}

// FUNÇÃO PARA CRIAR UM NOVO TREINAMENTO (ABA TREINAMENTOS)
document.getElementById('treinamentoForm').addEventListener('submit', function (e) {
  ...
  authenticatedFetch('/api/treinamentos', {
    method: 'POST',
...
function carregarTreinamentos() {
  ...
  return authenticatedFetch('/api/treinamentos')
    .then(res => {
...
// FUNÇÃO PARA RENDERIZAR OS CARDS DE TREINAMENTOS (ABA TREINAMENTOS)
      ...
      <div class="training-card">
        <div class="training-header">
          ...
          <div class="card-badge">
              <div class="tipo-treinamento">
                  <small><i class="fa-solid fa-helmet-safety"></i>${treinamento.tipo || 'N/A'}</small>
              </div>
          </div>
        </div>
        
        <div class="janela-scroll">
          <p class="conteudo">${treinamento.conteudo_programatico || 'Sem conteúdo programático'}</p>
        </div>
        ...
        <div class="training-details">
          <div class="training-info">
            <p>
              ...
              ${treinamento.modalidade || 'N/A'}
            </p>
            <br>
            <p>
              <strong>Carga horária: </strong>
              ${treinamento.carga_horaria || 'N/A'}h
            </p>
            <br>
            <p>
              <strong>Instrutor: </strong>
              ${treinamento.instrutor_principal || 'N/A'}
            </p>
          </div>
        </div>

        <div class="training-actions">
          <button onclick="visualizarContatosTreinamento(${treinamento.id})">
            <i class="fa-solid fa-user"></i>
            Contatos
          </button>
          ...
          <button onclick="excluirTreinamento(${treinamento.id})">
            <i class="fa-solid fa-trash"></i>
            Excluir
          </button>
        </div>
...
function abrirDetalhesTreinamento(treinamento_id) {
  ...
  <form id="editarTreinamentoForm">
    <div class="form-section">
      ...
      <div class="form-group">
        <label for="editarNomeTreinamento">Nome do Treinamento <span class="required">*</span></label>
        <div class="input-wrapper">
          <div class="input-icon">
            <i class="fa-solid fa-graduation-cap"></i>
          </div>
      ...
      <div class="form-row">
        <div class="form-group">
          <label for="editarModalidadeTreinamento">Modalidade de Treinamento <span class="required">*</span></label>
          <div class="input-wrapper">
            <div class="input-icon">
              <i class="fa-solid fa-chalkboard-user"></i>
            </div>
            <select id="editarModalidadeTreinamento" required>
              <option value="" disabled hidden>Selecione...</option>
              <option value="Presencial" ${treinamento.modalidade === "Presencial" ? "selected" : ""}>Presencial</option>
              <option value="Online" ${treinamento.modalidade === "Online" ? "selected" : ""}>Online</option>
              <option value="Hibrido" ${treinamento.modalidade === "Hibrido" ? "selected" : ""}>Híbrido</option>
              <option value="EAD" ${treinamento.modalidade === "EAD" ? "selected" : ""}>EAD</option>
            </select>
          </div>
        </div>
        <div class="form-group">
          ...
          <div class="input-wrapper">
            ...
            <input type="number" id="editarCargaHoraria" value="${treinamento.carga_horaria || ''}" required />
          </div>
        </div>
        <div class="form-group">
          <label for="editarTipoTreinamento">Tipo de Treinamento <span class="required">*</span></label>
          <div class="input-wrapper">
            <div class="input-icon">
              <i class="fa-solid fa-book"></i>
            </div>
            <select id="editarTipoTreinamento" required>
              <option value="" disabled hidden>Selecione...</option>
              <option value="Teórico" ${treinamento.tipo === "Teórico" ? "selected" : ""}>Teórico</option>
              <option value="Prático" ${treinamento.tipo === "Prático" ? "selected" : ""}>Prático</option>
              <option value="Teórico e Prático" ${treinamento.tipo === "Teórico e Prático" ? "selected" : ""}>Teórico e Prático</option>
            </select>
          </div>
        </div>
      </div>
      <div class="form-group">
        <label for="editarEmConformidade">Em Conformidade <span class="required">*</span></label>
        <div class="input-wrapper">
          <div class="input-icon">
            <i class="fa-solid fa-check-to-slot"></i>
          </div>
          <textarea id="editarEmConformidade" maxlength="255" required>${treinamento.em_conformidade || ''}</textarea>
        </div>
      </div>
      <div class="form-group">
        <label for="editarAproveitamento">Aproveitamento de Conteúdo <span class="required">*</span></label>
        <div class="input-wrapper">
          <div class="input-icon">
            <i class="fa-solid fa-chart-simple"></i>
          </div>
          <textarea id="editarAproveitamento" required>${treinamento.aproveitamento_conteudo || ''}</textarea>
        </div>
      </div>
      <div class="form-group">
        <label for="editarConteudoTreinamento">Conteúdo Programático Aplicado <span class="required">*</span></label>
        <div class="input-wrapper">
          <div class="input-icon">
            <i class="fa-solid fa-clipboard-list"></i>
          </div>
          <textarea id="editarConteudoTreinamento" rows="3" required>${treinamento.conteudo_programatico || ''}</textarea>
        </div>
      </div>
    </div>
    ...
    <div class="form-section">
      <h4>
        ...
        INSTRUTOR E RESPONSÁVEL
      </h4>
      <div class="form-group">
        <label for="editarInstrutor">Nome do Instrutor Principal <span class="required">*</span></label>
        <div class="input-wrapper">
          <div class="input-icon">
            <i class="fa-solid fa-user-check"></i>
          </div>
          <input type="text" id="editarInstrutor" required value="${treinamento.instrutor_principal || ''}" />
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label for="editarQualificacaoInstrutor">Qualificação do Instrutor</label>
          <div class="input-wrapper">
            <div class="input-icon">
              <i class="fa-solid fa-award"></i>
            </div>
            <input type="text" id="editarQualificacaoInstrutor" value="${treinamento.qualificacao_instrutor || ''}" />
          </div>
        </div>
        <div class="form-group">
          <label for="editarRegistroInstrutor">Registro do Instrutor <span class="required">*</span></label>
          <div class="input-wrapper">
            <div class="input-icon">
              <i class="fa-solid fa-file-signature"></i>
            </div>
            <input type="text" id="editarRegistroInstrutor" required value="${treinamento.registro_instrutor || ''}" />
          </div>
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label for="editarResponsavel">Responsável pelo Treinamento <span class="required">*</span></label>
          <div class="input-wrapper">
            <div class="input-icon">
              <i class="fa-solid fa-user-shield"></i>
            </div>
            <input type="text" id="editarResponsavel" required value="${treinamento.responsavel_treinamento || ''}" />
          </div>
        </div>
        <div class="form-group">
          <label for="editarCargoResponsavel">Cargo do Responsável</label>
          <div class="input-wrapper">
            <div class="input-icon">
              <i class="fa-solid fa-user-shield"></i>
            </div>
            <input type="text" id="editarCargoResponsavel" value="${treinamento.cargo_responsavel || ''}" />
          </div>
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label for="editarAreaResponsavel">Área do Responsável <span class="required">*</span></label>
          <div class="input-wrapper">
            <div class="input-icon">
              <i class="fa-solid fa-user-shield"></i>
            </div>
            <input type="text" id="editarAreaResponsavel" required value="${treinamento.area_responsavel || ''}" />
          </div>
        </div>
        <div class="form-group">
          <label for="editarRegistroResponsavel">Registro do Responsável <span class="required">*</span></label>
          <div class="input-wrapper">
            <div class="input-icon">
              <i class="fa-solid fa-user-shield"></i>
            </div>
            <input type="text" id="editarRegistroResponsavel" required value="${treinamento.registro_responsavel || ''}" />
          </div>
        </div>
      </div>
    </div>
    
    <div class="form-section" id="midia-treinamento">
      <h4>
        <i class="fa-solid fa-folder-open"></i>
        MÍDIAS DO TREINAMENTO
      </h4>
        ...
        <div class="file-upload-container">
          <div class="file-upload-area" onclick="document.getElementById('novasMidias').click()">
            <div class="upload-icon">
              <i class="fa-solid fa-folder-open" style="color: #F1AE33"></i>
            </div>
            <div class="upload-text">Clique aqui ou arraste os arquivos</div>
            <div class="upload-hint">
              <i class="fa-solid fa-circle-info"></i>
              Tamanho máximo permitido (100MB cada)
            </div>
...
`;
  ...
  document.getElementById('modalDetalhesTreinamento').style.display = 'flex';
}
...
// FUNÇÃO PARA FECHAR MODAL DE DETALHES DO TREINAMENTO
...
function excluirTreinamento(id) {
  ...
  if (contatosComTreinamento.length > 0) {
    if (!confirm(`Este treinamento possui ${contatosComTreinamento.length} contatos. Ao excluí-lo, os contatos perderão a associação com o treinamento. Deseja continuar?`)) {
      return;
    }
  } else {
  ...
  authenticatedFetch(`/api/treinamentos/${id}`, {
    method: 'DELETE'
  })
    .then(res => {
      if (!res.ok) throw new Error();
      mostrarAlerta('Treinamento excluido com sucesso!');
      carregarTreinamentos().then(() => {
        atualizarSelectTreinamento();
    ...
    .catch(() => mostrarAlerta('Erro ao excluir treinamento.', 'error'));
}
...
function carregarEmpresas() {
  ...
  return authenticatedFetch('/api/empresas')
    .then(res => {
...
document.getElementById('empresaForm')?.addEventListener('submit', function (e) {
  ...
  authenticatedFetch('/api/empresas', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  container.innerHTML = `
    ...
    ${Array.from(arquivos).map((arquivo, index) => `
      <div class="file-item" data-index="${index}">
        ...
        <button type="button" class="file-remove" onclick="removerArquivo(${index}, '${containerId}')">
...
// Função para exportar conversa do WhatsApp
function exportarConversa(contatoId) {
  const contato = contatos.find(c => c.id === contatoId);
  if (!contato) {
    mostrarAlerta('Contato não encontrado.', 'error');
    return;
