// Script de treinamento: Nr10 - Curso Básico - Atualização
...
/**
 * Executa o treinamento: Nr10 - Curso Básico - Atualização
 */
async function executarTreinamento(sender, contato) {
    const treinamento = await Treinamento.findOne({
        where: { nome: { [require('sequelize').Op.like]: '%NR10%' } }
    });
...
async function processarRespostaTeste(sender, text, selectedId, contato) {
    ...
    if ((text === '2' || textLower === '2') && ultimaInteracao?.tipo === 'aguardando_numero2_nr10') {
        ...
        await sendMessage(sender, 'send-message', {
            message: '*Para o quiz final, digite o número 3️⃣*',
        });

        await salvarInteracao(sender, 'aguardando_numero3_nr10', '*Para o quiz final, digite o número 3️⃣*');
        return true;
    }
...
async function gerarEEnviarCertificadoNR10(contato, sender) {
    try {
        ...
        await sendMessage(sender, 'send-file', {
            ...
            caption: '🎓 Seu certificado de conclusão do NR-10'
        });
