...
// ==================== FUNÇÃO AUXILIAR DE PAUSA (CORREÇÃO) ====================
/**
 * Cria uma pausa na execução de uma função async.
...
async function processarOpcaoInicial(sender, text, contato, sendMessage) {
    ...
    if (opcao === '1' || opcao.toLowerCase().includes('sim')) {
        if (!contato) {
            await delay(300); // CORREÇÃO: Usando delay
            await sendMessage(sender, 'send-message', {
        ...
        await delay(500); // CORREÇÃO: Usando delay
        try {
...
async function mostrarComoFunciona(sender, nome, sendMessage) {
    ...
    await delay(800); // CORREÇÃO: Usando delay
...
async function mostrarRecursosDetalhados(sender, sendMessage) {
    try {
        ...
        await delay(1500); // CORREÇÃO: Usando delay

        // 1. Enviar vídeo
        const videoPath = path.join(__dirname, 'material_apresentacao', 'Videos', 'Video01.mp4');
        if (fs.existsSync(videoPath)) {
            console.log(`🎥 Enviando vídeo: ${videoPath}`);
            await sendMessage(sender, 'send-video', { path: videoPath, caption: '📹 Vídeos curtos' });
            console.log('✅ Vídeo enviado com sucesso');
        }

        await delay(2000); // CORREÇÃO: Usando delay

        // 2. Enviar imagem
        const imagePath = path.join(__dirname, 'material_apresentacao', 'Imagens', 'Vantagens.png');
        if (fs.existsSync(imagePath)) {
            await sendMessage(sender, 'send-image', { path: imagePath, caption: '🖼️ Imagens e infográficos' });
            console.log('✅ Imagem enviada com sucesso');
        }

        await sendMessage(sender, 'send-message', { message: '• 🎤 Áudios explicativos' });
        await sendMessage(sender, 'send-message', { message: 'Áudio com o texto: Já imaginou fazermos um treinamento interativo, simples, com linguagem clara e cheio de Interação? É isso que você terá a oportunidade de participar com os treinamentos normativos no WhatsApp' });
        ...
        if (fs.existsSync(audioPath)) {
            console.log(`🎵 Enviando áudio: ${audioPath}`);
            await sendMessage(sender, 'send-file', { path: audioPath, filename: 'audio.mp3', caption: ' Áudios explicativos' });
            console.log('✅ Áudio enviado com sucesso');
        }

        await delay(2000); // CORREÇÃO: Usando delay

        // 4. Enviar mensagem de testes e pergunta
        await sendMessage(sender, 'send-message', { message: '• 📝 Testes e avaliações\n\nVocê concorda em realizar treinamentos normativos no WhatsApp em sua empresa? (Texto, também em áudio)' });
        console.log('✅ Mensagem de testes e avaliações enviada');
        
        await sendMessage(sender, 'send-message', { message: 'Você concorda em realizar treinamentos normativos no WhatsApp em sua empresa?\n\n1️⃣ SIM\n2️⃣ COM CERTEZA' });
        console.log('✅ SUCESSO: Mensagem de testes enviada');

        // 5. Salvar a interação DEPOIS de enviar tudo
        await salvarInteracao(sender, 'testes_avaliacoes', JSON.stringify({ etapa: 'testes_avaliacoes' }));
        console.log('✅ SUCESSO: Interação testes_avaliacoes salva');

    } catch (error) {
        console.error('❌ Erro ao enviar arquivos em mostrarRecursosDetalhados:', error);
    }
}
...
async function mostrarQuandoOnde(sender, sendMessage) {
    try {
        ...
        if (fs.existsSync(imagemCafe)) {
            await sendMessage(sender, 'send-image', { path: imagemCafe, caption: '☕ No intervalo do café' });
        }

        await delay(1000); // CORREÇÃO: Usando delay
        ...
        if (fs.existsSync(imagemMetro)) {
            await sendMessage(sender, 'send-image', { path: imagemMetro, caption: '🚎 No ônibus ou metrô' });
        }
        
        await delay(1000); // CORREÇÃO: Pausa adicional para dar tempo entre as imagens
        ...
        if (fs.existsSync(imagemTrabalho)) {
            await sendMessage(sender, 'send-image', { path: imagemTrabalho, caption: '🌍 Em qualquer lugar, a qualquer hora' });
        }
        
        await delay(1500); // CORREÇÃO: Usando delay

        // 4. Enviar mensagem após as imagens
        await sendMessage(sender, 'send-message', { message: 'Tudo com registro, certificado e validade normativa.' });
        ...
        if (fs.existsSync(audioPath)) {
            await sendMessage(sender, 'send-file', { path: audioPath, filename: 'norma-atendida.mp3', caption: ' Validade normativa' });
            console.log('✅ Áudio norma-atendida enviado com sucesso');
        }
        
        // A lógica agora vai para a próxima etapa naturalmente, aguardando a interação do usuário.

    } catch (error) {
        console.error('❌ Erro ao enviar imagens em mostrarQuandoOnde:', error);
    }
    
    // Após mostrar tudo, avança para a próxima etapa
    await delay(2000); // Pausa final antes de perguntar o próximo passo
    await mostrarExemplosTrainamentos(sender, sendMessage);
}
...
async function enviarVideoTreinamentoMotorista(sender, sendMessage) {
    try {
        ...
        console.error('❌ Erro ao enviar vídeo de treinamento:', error);
        await mostrarOutrasAplicacoes(sender, sendMessage); // Continua o fluxo mesmo em caso de erro
    }
}
...
async function mostrarOutrasAplicacoes(sender, sendMessage) {
    ...
    await delay(1200); // CORREÇÃO: Usando delay

    await finalizarApresentacao(sender, sendMessage);
}

async function processarOutrasAplicacoes(sender, text, sendMessage) {
    // Esta etapa agora leva diretamente para a finalização
    await finalizarApresentacao(sender, sendMessage);
    return true;
}
