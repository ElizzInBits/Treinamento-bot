...
function getMensagemQuizInicial() {
  return {
    title: '',
  ...
  client.onMessage(async message => {
    ...
    try {
      ...
      const contato = contatos.find(c =>
        gerarVariacoes(sender).some(num => gerarVariacoes(c.telefone).includes(num))
      );
      ...
      // === fluxo de "continuar de onde parei" ===
      ...
      if (text === 'não, começo assim que possível 👀 😅' || selectedId === 'não começar') {
        const listMsg = getMensagemQuizInicial();
        await sendMessage(sender, 'send-message', {
      ...
      if (['a','b','c','d'].includes(text)) {
        const correta = 'b';
        await sendMessage(sender, 'send-message', {
          message: text !== correta
            ? '❌ Resposta incorreta! A resposta correta é B) Segurança é de responsabilidade coletiva.'
            : '✅ Resposta correta! Segurança é de responsabilidade coletiva!',
        });
      ...
      console.error(`❌ Erro no processamento da mensagem de ${sender}:`, err);
    } finally {
      emProcessamento.delete(sender);
    }
  });
