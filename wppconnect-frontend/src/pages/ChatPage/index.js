...
const SendMessagePage = () => {
  ...
  // 1) Normaliza o id para string (_serialized existe sempre quando id é objeto)
  const contactId =
  ...
  setImgContact("");
  // 2) Guarda o contact já com id como string
  setChoosedContact({ ...contact, id: contactId });
  setOpenLoading(true);
  setAllMessages([]);
  setHasNoMore(false);
  ...
  try {
    if (contactId.includes("@g.us")) {
      const cleanId = contactId.replace("@g.us", "");
      const { data } = await api.get(
        `${getSession()}/chat-by-id/${cleanId}?isGroup=true`,
        config()
      );
      await api.post(
      ...
      const { data } = await api.get(
        `${getSession()}/chat-by-id/${cleanId}?isGroup=false`,
        config()
      );
      await api.post(
        `${getSession()}/send-seen`,
    ...
    // fecha o backdrop sempre, mesmo se der erro
